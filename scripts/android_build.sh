...
docker run --rm \
  ...
  bash -Eeuo pipefail -c '
    ...
    install -m 0644 helpify/build/outputs/apk/debug/helpify-debug.apk /out/Helpify-v0.10.0-debug.apk
    install -m 0644 mydealer/build/outputs/apk/debug/mydealer-debug.apk /out/MyDealer-v0.10.0-debug.apk

    aapt dump badging /out/Helpify-v0.10.0-debug.apk > /out/Helpify-v0.10.0-badging.txt
    aapt dump badging /out/MyDealer-v0.10.0-debug.apk > /out/MyDealer-v0.10.0-badging.txt

    cd /out
    sha256sum Helpify-v0.10.0-debug.apk MyDealer-v0.10.0-debug.apk > SHA256SUMS.txt
  '
