...
MODE="${1:-all}"
OUTPUT_DIR="${ANDROID_OUTPUT_DIR:-/var/lib/helpify-mydealer/android-builds/v0.12.0}"
PASS=0
WARN=0
FAIL=0
...
source_checks(){
  ...
  for module in helpify mydealer; do
    grep -Fq "versionCode 1200" "$ROOT/implementation/android-native/$module/build.gradle" \
      && pass "$module versionCode 1200" || fail "$module versionCode 1200"
    grep -Fq "versionName '0.12.0'" "$ROOT/implementation/android-native/$module/build.gradle" \
      && pass "$module versionName 0.12.0" || fail "$module versionName 0.12.0"
  done
...
artifact_checks(){
  local helpify="$OUTPUT_DIR/Helpify-v0.12.0-debug.apk"
  local mydealer="$OUTPUT_DIR/MyDealer-v0.12.0-debug.apk"
  ...
  check_file "$OUTPUT_DIR/Helpify-v0.12.0-badging.txt" "Helpify APK metadata"
  check_file "$OUTPUT_DIR/MyDealer-v0.12.0-badging.txt" "MyDealer APK metadata"
